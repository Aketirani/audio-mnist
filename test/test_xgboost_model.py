...
class TestXGBoostModel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.train_df = pd.DataFrame({'col1': [1, 2, 3, 4, 5, 6],
                                      'col2': [1, 2, 3, 4, 5, 6],
                                      'label': [0, 1, 0, 1, 0, 1]})
        self.val_df = pd.DataFrame({'col1': [7, 8, 9, 10, 11, 12],
                                    'col2': [7, 8, 9, 10, 11, 12],
                                    'label': [0, 1, 0, 1, 0, 1]})
        self.test_df = pd.DataFrame({'col1': [13, 14, 15, 16, 17, 18],
                                     'col2': [13, 14, 15, 16, 17, 18],
                                     'label': [0, 1, 0, 1, 0, 1]})
        self.xgboost_model = XGBoostModel(self.train_df, self.val_df, self.test_df)
        self.model_param = {"learning_rate": 0.05,
                            "max_depth": 5,
                            "n_estimators": 500,
                            "gamma": 0,
                            "lambda": 1,
                            ...
                            "tree_method": "hist"}
        self.log_data = {"validation_0":
                            {"logloss": [0.5, 0.4, 0.3],
                             "accuracy": [0.3, 0.4, 0.5]},
                         "validation_1":
                            {"logloss": [0.4, 0.3, 0.2],
                             "accuracy": [0.2, 0.3, 0.4]}}
        self.expected_df = pd.DataFrame({"iteration": [0, 1, 2],
    ...
    def test_prepare_data(self):
        ...
        X_train, y_train, X_val, y_val, X_test, y_test = self.xgboost_model.prepare_data()
    ...
    def test_set_params(self):
        ...
        self.assertEqual(self.xgboost_model.model.learning_rate, self.model_param["learning_rate"])
        self.assertEqual(self.xgboost_model.model.max_depth, self.model_param["max_depth"])
        self.assertEqual(self.xgboost_model.model.n_estimators, self.model_param["n_estimators"])
        self.assertEqual(self.xgboost_model.model.gamma, self.model_param["gamma"])
        self.assertEqual(self.xgboost_model.model.reg_lambda, self.model_param["lambda"])
        self.assertEqual(self.xgboost_model.model.scale_pos_weight, self.model_param["scale_pos_weight"])
        self.assertEqual(self.xgboost_model.model.min_child_weight, self.model_param["min_child_weight"])
        self.assertEqual(self.xgboost_model.model.objective, self.model_param["objective"])
        self.assertEqual(self.xgboost_model.model.tree_method, self.model_param["tree_method"])
    ...
    def test_evaluate_predictions(self):
        ...
        result = self.xgboost_model.evaluate_predictions(self.val_df["label"].values, self.test_df["label"].values)
